...
@endif
        ...
        <br>
        <form method="post" action="{{ route('module.update', $selectedModule->id) }}">
            @csrf
            @method('PUT')
            <div class="row mb-3">
                    <label class="col-form-label col-sm-1" for="nom">Nom:</label>
                    <div class="col-sm-6">
                        <input value="{{ $selectedModule->nom }}" class="form-control @error('nom') is-invalid @enderror" type="text" id="nom" name="nom" required>
                        @error('nom')
            ...
            <div class="row mb-3">
                    ...
                    <div class="col-sm-6">
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description">{{ $selectedModule->description }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-sm-6">
                        <select class="form-select @error('filiere_id') is-invalid @enderror" id="filiere_id" name="filiere_id" required>
                            ...
                            @foreach($filieres as $filiere)
                                <option value="{{ $filiere->id }}" {{ $selectedModule->filiere_id == $filiere->id ? 'selected' : '' }}>{{ $filiere->nom }}</option>
                            @endforeach
                        </select>
                        @error('filiere_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-sm-6">
                        <select class="form-select @error('professeur_id') is-invalid @enderror" id="professeur_id" name="professeur_id" required>
                            ...
                            @foreach($professeurs as $professeur)
                                <option value="{{ $professeur->id }}" {{ $selectedModule->professeur_id == $professeur->id ? 'selected' : '' }}>{{ $professeur->name }}</option>
                            @endforeach
                        </select>
                        @error('professeur_id')
                            <div class="invalid-feedback">{{ $message }}</div>
