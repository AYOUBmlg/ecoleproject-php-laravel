...
@endif
        ...
        <br>
        <form method="post" action="{{ route('cour.update', $selectedCour->id) }}">
            @csrf
            @method('PUT')
            <div class="row mb-3">
                    <label class="col-form-label col-sm-1" for="titre">Titre:</label>
                    <div class="col-sm-6">
                        <input value="{{ $selectedCour->titre }}" class="form-control @error('titre') is-invalid @enderror" type="text" id="titre" name="titre" required>
                        @error('titre')
            ...
            <div class="row mb-3">
                    ...
                    <div class="col-sm-6">
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description">{{ $selectedCour->description }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="row mb-3">
                    ...
                    <div class="col-sm-6">
                        <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                            ...
                            <option value="Cours" {{ $selectedCour->type == 'Cours' ? 'selected' : '' }}>Cours</option>
                            <option value="TD" {{ $selectedCour->type == 'TD' ? 'selected' : '' }}>TD</option>
                            <option value="TP" {{ $selectedCour->type == 'TP' ? 'selected' : '' }}>TP</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
            ...
            <div class="row mb-3">
                    ...
                    <div class="col-sm-6">
                        <input value="{{ $selectedCour->fichier }}" class="form-control @error('fichier') is-invalid @enderror" type="text" id="fichier" name="fichier" required>
                        @error('fichier')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-sm-6">
                        <select class="form-select @error('module_id') is-invalid @enderror" id="module_id" name="module_id" required>
                            ...
                            @foreach($modules as $module)
                                <option value="{{ $module->id }}" {{ $selectedCour->module_id == $module->id ? 'selected' : '' }}>{{ $module->nom }}</option>
                            @endforeach
                        </select>
                        @error('module_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-sm-6">
                        <select class="form-select @error('filiere_id') is-invalid @enderror" id="filiere_id" name="filiere_id" required>
                            ...
                            @foreach($filieres as $filiere)
                                <option value="{{ $filiere->id }}" {{ $selectedCour->filiere_id == $filiere->id ? 'selected' : '' }}>{{ $filiere->nom }}</option>
                            @endforeach
                        </select>
                        @error('filiere_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-sm-6">
                        <select class="form-select @error('professeur_id') is-invalid @enderror" id="professeur_id" name="professeur_id" required>
                            ...
                            @foreach($professeurs as $professeur)
                                <option value="{{ $professeur->id }}" {{ $selectedCour->professeur_id == $professeur->id ? 'selected' : '' }}>{{ $professeur->name }}</option>
                            @endforeach
                        </select>
                        @error('professeur_id')
                            <div class="invalid-feedback">{{ $message }}</div>
